...
if __name__ == '__main__':
    ...
    for key, value in goods_infos_dict.items():
        ...
        if location in data:
            ...
            data[location] = 1
    drawMap(title='淘宝上卖{}的商家在全国范围内的数量​分布情况'.format(name), data=data)
    # 淘宝上卖的​店铺的销量排名前10
    data = {}
    for key, value in goods_infos_dict.items():
    ...
    drawBar('淘宝上卖{}的​店铺的销量排名前10'.format(name), data)
    # 淘宝上评论数量前10名的奶茶店铺
    data = {}
    for key, value in goods_infos_dict.items():
        ...
        if value['shope_name'] in data:
            ...
            data[value['shope_name']] = num_comments
    data = dict(sorted(data.items(), key=lambda item: item[1])[-10:])
    drawBar('淘宝上评论数量前10名的{}店铺'.format(name), data)
    # 要运费与免运费的奶茶相关商品比例
    data = {'fee': 0, 'no_fee': 0}
    for key, value in goods_infos_dict.items():
        if float(value['fee']) > 0:
            ...
            data['no_fee'] += 1
    drawPie('要运费与免运费的{}相关商品比例'.format(name), data)
    # 奶茶相关商品的售价区间
    ...
    for key, value in goods_infos_dict.items():
        ...
        if price < 10:
            ...
            data['100元以上'] += 1
    drawPie('{}相关商品的售价区间'.format(name), data)
